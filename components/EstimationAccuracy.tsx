...
export default function EstimationAccuracy() {
  ...
  const calculateAccuracyMetrics = () => {
    ...
    const accuracyData = completedFeatures.map((feature: any) => {
      const original =
        feature.estimation?.originalEstimate || feature.storyPoints;
      ...
      const estimationAccuracy =
        final > 0
          ? Math.round((1 - Math.abs(final - actual) / final) * 100)
          : 0;
      const variancePercentage =
        final > 0 ? Math.round(((actual - final) / final) * 100) : 0;
    ...
    const totalPlanned = accuracyData.reduce(
  ...
  const teamPerformanceData = features.reduce((acc: any, feature: any) => {
    ...
    if (feature.status === "completed") {
      ...
      acc[member].plannedSP +=
        feature.estimation?.finalEstimate || feature.storyPoints;
  ...
  Object.values(teamPerformanceData).forEach((member: any) => {
    member.accuracy =
      member.plannedSP > 0
        ? Math.round(
            (1 -
              Math.abs(member.plannedSP - member.actualSP) / member.plannedSP) *
              100
          )
        : 0;
  });
          ...
          <div className="text-center p-4 bg-gradient-to-br from-white to-blue-50 border border-blue-100 rounded-lg">
            ...
            <p
              className={`text-2xl font-bold ${
                overallVariance > 0 ? "text-red-700" : "text-green-700"
              }`}
            >
              {overallVariance > 0 ? "+" : ""}
              {overallVariance}%
          ...
          <TabsContent value="team" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.values(teamPerformanceData).map((member: any, index) => (
                <div
                  key={index}
                  className="p-4 bg-gradient-to-br from-white to-blue-50 border border-blue-100 rounded-lg"
                >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-green-900 font-medium">
                      {projectEstimation.estimationMetadata.estimationMethod}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={`${
                        projectEstimation.riskLevel === "Low"
                          ? "bg-green-100 text-green-800 border-green-300"
                          : projectEstimation.riskLevel === "Medium"
                          ? "bg-yellow-100 text-yellow-800 border-yellow-300"
                          : "bg-red-100 text-red-800 border-red-300"
                      } border`}
                    >
                ...
                <div className="space-y-2 text-sm text-orange-800">
                  {avgAccuracy < 85 && (
                    <div className="flex items-start space-x-2">
                      ...
                      <span>
                        Consider refining estimation techniques for better
                        accuracy
                      </span>
                    </div>
                  )}
                  {Math.abs(overallVariance) > 15 && (
                    <div className="flex items-start space-x-2">
                      <div className="w-1.5 h-1.5 bg-orange-500 rounded-full mt-2 flex-shrink-0"></div>
                      <span>
                        High variance detected - review scope and complexity
                        assessment
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex items-start space-x-2">
                    <div className="w-1.5 h-1.5 bg-orange-500 rounded-full mt-2 flex-shrink-0"></div>
                    <span>
                      Regular retrospectives can help identify estimation
                      patterns
                    </span>
                  </div>
                </div>
