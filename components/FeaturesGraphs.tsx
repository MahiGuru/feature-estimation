...
export default function FeaturesGraphs() {
  ...
  const getChartData = (feature: any): ChartData[] => {
    return Object.entries(feature.quarterlyConsumption).map(([quarter, data]: [string, any]) => ({
      quarter,
            ...
            <TabsContent key={feature.id} value={feature.id} className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-blue-900">{feature.name}</h3>
                  <p className="text-sm text-blue-600 mt-1">{feature.description}</p>
                </div>
                
                <div className="flex flex-wrap items-center gap-3">
                  <Badge className={`${
                    feature.size === 'S' ? 'bg-green-100 text-green-800 border-green-300' :
                    feature.size === 'M' ? 'bg-blue-100 text-blue-800 border-blue-300' :
                    feature.size === 'L' ? 'bg-purple-100 text-purple-800 border-purple-300' :
                    feature.size === 'XL' ? 'bg-orange-100 text-orange-800 border-orange-300' :
                  ...
                  <Badge className={`${
                    feature.status === 'completed' ? 'bg-green-100 text-green-800 border-green-300' :
                    feature.status === 'in-progress' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' :
                    feature.status === 'blocked' ? 'bg-red-100 text-red-800 border-red-300' :
              ...
              <div className="bg-white p-4 rounded-lg border border-blue-100">
                {chartType === 'bar' ? renderBarChart(feature) : renderPieChart(feature)}
              </div>

              {/* Feature Statistics */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-gradient-to-br from-white to-blue-50 border border-blue-100 rounded-lg">
                  <p className="text-sm text-blue-600 mb-1">Total Story Points</p>
                  <p className="text-2xl font-bold text-blue-900">{feature.storyPoints}</p>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-white to-green-50 border border-green-100 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-700">{feature.consumedPoints}</p>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-white to-orange-50 border border-orange-100 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-orange-700">{feature.storyPoints - feature.consumedPoints}</p>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-white to-purple-50 border border-purple-100 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-purple-700">{feature.progress}%</p>
                </div>
              </div>
            </TabsContent>
          ))}
