...
export default function FeatureTracker() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge className={`${statusClasses[status as keyof typeof statusClasses]} px-3 py-1 text-xs font-medium rounded-full`}>
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return (
      <Badge className={`${priorityClasses[priority as keyof typeof priorityClasses]} px-2 py-1 text-xs rounded-full border`}>
  ...
  const handleSaveFeature = () => {
    if (selectedFeature) {
      setFeatures(features.map(f => f.id === selectedFeature.id ? selectedFeature : f));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
          ...
          <p className="text-blue-600">Track and manage project features with detailed insights</p>
        </div>
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-2">
              <Label className="text-sm font-medium text-blue-800">Search</Label>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-blue-100">
                  ...
                  <th className="text-left p-4 font-semibold text-blue-900">Priority</th>
                  <th className="text-left p-4 font-semibold text-blue-900">Progress</th>
                  <th className="text-left p-4 font-semibold text-blue-900">Story Points</th>
                  <th className="text-left p-4 font-semibold text-blue-900">Assigned To</th>
                  <th className="text-left p-4 font-semibold text-blue-900">Sprint</th>
                  <th className="text-left p-4 font-semibold text-blue-900">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredFeatures.map((feature) => (
                  <tr key={feature.id} className="table-row-hover border-b border-blue-50">
                    <td className="p-4">
                      <div>
                    ...
                    <td className="p-4">
                      <div className="space-y-2">
                        ...
                        <span className="text-xs text-blue-600">{feature.progress}%</span>
                      </div>
                    </td>
                    <td className="p-4">
                          ...
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              <DialogTitle className="text-blue-900">
                                {isEditMode ? 'Edit Feature' : 'Feature Details'}
                              </DialogTitle>
                            </DialogHeader>
                            {selectedFeature && (
                              <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <Label className="text-blue-800">Feature Name</Label>
                                    {isEditMode ? (
                                      <Input
                                      ...
                                      <p className="text-blue-700">{selectedFeature.name}</p>
                                    )}
                                  </div>
                                  <div>
                                    <Label className="text-blue-800">Assigned To</Label>
                                    {isEditMode ? (
                                      <Input
                                      ...
                                      <p className="text-blue-700">{selectedFeature.assignedTo}</p>
                                    )}
                                  </div>
                                </div>
                                <div>
                                  ...
                                  {isEditMode ? (
                                    ...
                                    <p className="text-blue-700">{selectedFeature.description}</p>
                                  )}
                                </div>
                                <div className="grid grid-cols-3 gap-4">
                                  <div>
                                    ...
                                    {isEditMode ? (
                                      <Select
                                        ...
                                        onValueChange={(value) => setSelectedFeature({
                                          ...selectedFeature,
                                          status: value
                                        })}
                                      >
                                  ...
                                  <div>
                                    <Label className="text-blue-800">Story Points</Label>
                                    {isEditMode ? (
                                      <Input
                                        type="number"
                                      ...
                                      <p className="text-blue-700">{selectedFeature.storyPoints} SP</p>
                                    )}
                                  </div>
                                </div>
                                <div>
                                ...
                                {isEditMode && (
                                  <div className="flex space-x-2 pt-4">
                                    <Button onClick={handleSaveFeature} className="gradient-button">
                                      Save Changes
                                    </Button>
                                    <Button
                                      variant="outline"
